...
export default function Navbar({ isLoaded, onLoginClick, onSignUpClick }) {
  ...
  // Contact refs removed as they are replaced by Login/Sign Up buttons
      ...
      >
        <div className="max-w-7xl mx-auto px-6 md:px-12 flex justify-between items-center">
          ...
          <a href="#/" className="flex items-center gap-2 group/logo interactive-hover">
            <svg viewBox="0 0 24 24" fill="none" className="w-6 h-6 text-red-600 dark:text-neon group-hover/logo:rotate-45 transition-transform duration-500">
          ...
          <ul className="hidden md:flex items-center gap-10">
            {navLinks.map((link, idx) => (
              <li key={idx} className="overflow-hidden h-6 relative group">
                ...
                <a 
                  href={link.href}
                  className="block font-sans text-sm font-medium tracking-wide text-red-600 dark:text-neon absolute top-full left-0 transition-transform duration-300 group-hover:-translate-y-full"
                >
                  {link.label}
                </a>
              </li>
          ...
          <div className="hidden md:flex items-center gap-6">
            ...
            <div className="flex items-center gap-4">
              <button 
                ...
                className="overflow-hidden h-6 relative group flex items-center justify-center font-sans text-sm font-semibold px-4 cursor-pointer"
              >
                <span className="block text-neutral-600 dark:text-neutral-300 group-hover:text-black dark:group-hover:text-white transition-transform duration-300 group-hover:-translate-y-full">
                  Login
                </span>
                <span className="block text-red-600 dark:text-neon absolute top-full transition-transform duration-300 group-hover:-translate-y-full">
                  Login
                </span>
              </button>
              <button 
                onClick={onSignUpClick}
                className="h-10 bg-black dark:bg-white text-white dark:text-black hover:bg-neutral-800 dark:hover:bg-neutral-100 rounded-full pl-5 pr-4 flex items-center gap-2 group/signup font-sans text-sm font-semibold transition-all duration-300 border border-black/10 dark:border-white/10 hover:scale-105 active:scale-[0.98] hover:shadow-[0_0_20px_rgba(186,255,57,0.3)] dark:hover:shadow-[0_0_20px_rgba(186,255,57,0.2)] cursor-pointer shadow-md"
              >
                <span>Sign up</span>
                <span className="w-5 h-5 bg-red-600 dark:bg-neon rounded-full flex items-center justify-center text-white dark:text-black">
                  <ArrowUpRight className="w-3.5 h-3.5 group-hover/signup:translate-x-0.5 group-hover/signup:-translate-y-0.5 transition-transform duration-300" />
                </span>
              </button>
            </div>
          </div>
      ...
      >
        ...
        <div className="flex flex-col gap-4">
          <button 
            ...
            className="w-full h-12 border border-white/10 text-white rounded-full font-sans text-sm font-semibold flex items-center justify-center bg-white/5 active:scale-95 transition-transform"
          >
            Login
          </button>
          <button 
            onClick={() => { setMobileMenuOpen(false); onSignUpClick(); }}
            className="w-full h-12 bg-white text-black rounded-full font-sans text-sm font-semibold flex items-center justify-center active:scale-95 transition-transform"
          >
            Sign up
          </button>
          <div className="text-center text-xs text-white/30 tracking-[0.2em] mt-2">
